...
DUMPS_OUTPUT_FILE="pals/dump-arabe.txt"
CONTEXTES_OUTPUT_FILE="pals/contextes-arabe.txt"

# Créer le dossier pals si nécessaire
mkdir -p pals

# Vérification des paramètres
if [[ -z "$DUMPS_DIR" || -z "$CONTEXTES_DIR" ]]; then
    echo "Usage: $0 <dossier dumps-text> <nom de base> <dossier contextes>"
    exit 1
fi
