...
def analyze_cooccurrences(input_file, keyword, window_size=2):
    """
...
def save_comparison(local_stats, global_stats, local_keyword_count, global_keyword_count, output_file):
    ...
    Args:
        ...
        local_keyword_count (int): Nombre total d'apparitions du mot-clé "liberté" dans la fenêtre locale
        global_keyword_count (int): Nombre total d'apparitions du mot-clé "liberté" dans le corpus global
...
def main():
    # Chemins des fichiers avec chemins absolus
    base_dir = "/home/seif/Groupe PPE"
    contextes_file = os.path.join(base_dir, "pals/contextes-français.txt")
    dump_file = os.path.join(base_dir, "pals/dump-français.txt")
    output_file = os.path.join(base_dir, "pals/comparaison_resultats-français.txt")
    ...
    local_keyword_count = local_stats.get(keyword, 0)  # Nombre d'apparitions du mot-clé "liberté" dans la fenêtre locale
    ...
    global_keyword_count = global_stats.get(keyword, 0)  # Nombre d'apparitions du mot-clé "liberté" dans le corpus global
